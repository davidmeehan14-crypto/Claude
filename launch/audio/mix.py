...
import os, sys, subprocess, numpy as np, soundfile as sf, imageio_ffmpeg, pyloudnorm as pyln
...
MUSIC = sys.argv[1] if len(sys.argv) > 1 else os.path.join(HERE, 'stems', 'music.wav')
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, 'soundtrack.wav')
def loadp(p):
    x, sr = sf.read(p, always_2d=True); assert sr == SR
    o = np.zeros((N, 2)); o[:min(N, len(x))] = x[:N]; return o
music, sfx = loadp(MUSIC), load('sfx.wav')
...
out = OUT; sf.write(out, mix.astype(np.float32), SR, subtype='PCM_16')
if len(sys.argv) <= 2: subprocess.run([imageio_ffmpeg.get_ffmpeg_exe(), '-y', '-loglevel', 'error', '-i', out, '-c:a', 'aac', '-b:a', '256k', os.path.join(HERE, '..', 'soundtrack.m4a')], check=True)
