...
// usage: node render.mjs [--workers=4] [--from=0] [--to=60] [--out=out/the_wedding_chapter_launch_film.mp4] [--crf=16]
...
const FPS = 30, from = +(flags.from || 0), to = +(flags.to || 60), workers = +(flags.workers || 4), crf = flags.crf || '16';
const out = path.resolve(here, flags.out || 'out/the_wedding_chapter_launch_film.mp4');
...
async function worker(k) {
  ...
  for (let f = a; f < b; f++) {
    const d = await page.evaluate(t => { FILM.renderFrame(t); return document.getElementById('c').toDataURL('image/jpeg', 0.96); }, f / FPS);
