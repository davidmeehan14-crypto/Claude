...
const r = await p.evaluate(() => { const out = {}; const c = document.getElementById('c');
  for (let t = 0; t < 46; t += 1/30) { const a = performance.now(); FILM.renderFrame(t); c.getContext('2d').getImageData(0,0,1,1); const d = performance.now() - a; const k = Math.floor(t); out[k] = out[k] || []; out[k].push(d); }
